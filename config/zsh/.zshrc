...
if [[ -s "$NVM_DIR/nvm.sh" ]]; then
  # Create a function to initialize NVM once
  _init_nvm() {
    if [[ -z "$_NVM_INITIALIZED" ]]; then
      export NVM_DIR="${NVM_DIR:-${HOME}/.nvm}"
      [ -s "$NVM_DIR/nvm.sh" ] && \. "$NVM_DIR/nvm.sh"
      [ -s "$NVM_DIR/bash_completion" ] && \. "$NVM_DIR/bash_completion"
      export _NVM_INITIALIZED=1
    fi
  }
...
if [[ -s "${HOME}/.sdkman/bin/sdkman-init.sh" ]]; then
  # Create a function to initialize SDKMAN once
  _init_sdkman() {
    if [[ -z "$_SDKMAN_INITIALIZED" ]]; then
      source "${HOME}/.sdkman/bin/sdkman-init.sh"
      export _SDKMAN_INITIALIZED=1
    fi
  }
